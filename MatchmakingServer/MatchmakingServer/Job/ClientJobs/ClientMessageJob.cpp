...
void ClientMessageJob::HandleRequestGamemodeConnectionMessage()
{
	...
	GameSession->CV_AwaitGameSessionFill.wait(Lock, [GameSession]() { return GameSession->GetGameSessionState() == EGameSessionState::InProgress; } );

	// This should only happen when a thread that checks the state of game sessions decides that session is good to go.
	std::string Message;
	Message += "RGC|" + Session.lock()->GetServerInstanceIPnPort();
